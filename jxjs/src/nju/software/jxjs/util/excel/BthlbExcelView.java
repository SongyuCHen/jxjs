...
public class BthlbExcelView extends AbstractExcelView  
{
	...
	{
		// create sheet
        HSSFSheet sheet = workbook.createSheet("被退回列表");
        ...
        header.createCell(5).setCellValue("退回时间");
        header.getCell(5).setCellStyle(style);
        
        // create data item
        int rowCount = 1;
        List<BthlbView> bthlbList = (List<BthlbView>)model.get("bthlbList");
        if(bthlbList != null)
        {
        	for(BthlbView view : bthlbList)
        	{
                ...
                aRow.createCell(5).setCellValue(view.getThsj());
        	}
        }
        ...
        String filename = "被退回列表-"+curDateStr+".xls";
